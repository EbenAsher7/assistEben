...
const fetchChartData = async (token) => {
  try {
    const response = await fetch(`${URL_BASE}/admin/preguntasModulo`, {
      method: "GET",
      headers: {
...
const COLORS = ["#f4a582", "#92c5de", "#4393c3", "#2166ac", "#67001f", "#053061", "#ca0020"];

const PreguntasModulos = () => {
    ...
    if (user) {
      fetchChartData(user.token).then((data) => {
        const groupedData = data.reduce((acc, item) => {
          ...
          acc[item.tutor_nombre][item.modulo_nombre] = (acc[item.tutor_nombre][item.modulo_nombre] || 0) + item.cantidad_preguntas;
          return acc;
        }, {});
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription className="pt-2">
          Cantidad de preguntas que hacen los alumnos a lo largo del módulo, es la suma de los alumno de tipo virtual y presencial.
        </CardDescription>
      </CardHeader>
      <CardContent>
        {loading ? (
          <LoaderAE texto="Cargando datos de preguntas por módulo..." />
        ) : (
...
export default PreguntasModulos;
